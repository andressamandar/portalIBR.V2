...
import {Escala,EscalasService,SalvarEscalaRequest} from '../../../../core/services/escalas.service';
import {DisponibilidadesService,IntegranteDisponivel} from '../../../../core/services/disponibilidades.service';
...
export class CriarEscalaComponent implements OnInit {
  ...
  salvarEscala(): void {
    ...
    const dados: SalvarEscalaRequest = {
      ministerio: 'Louvor',
      data_id: data._id,
      data: data.data,
      funcoes
  ...
  verificarDuplicidade(
    ...
    const selecionados =
      this.selecoesPorFuncao[funcaoAtual] ?? [];

    for (const integrante of selecionados) {

      for (const outraFuncao of this.funcoesEscala) {

        if (outraFuncao === funcaoAtual) {
          continue;
        }

        const selecionadosOutraFuncao =
          this.selecoesPorFuncao[outraFuncao] ?? [];

        const repetido =
          selecionadosOutraFuncao.some(
            item =>
              item.id === integrante.id
          );
